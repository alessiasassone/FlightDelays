...
class Controller:
    ...
    def handleTestConnessione(self, e):
        # verifica se esiste connessione e in caso lo cerca
        ...
        self._view._txtResults.controls.append(
            ft.Text(f"Ho trovato un cammino tra {self._choicePartenza} e {self._choiceArrivo}. "
                    f"Di seguito i nodi che compongono il cammino: ",
                    color="green")
        )
        for p in path:
    ...
    def handleCerca(self, e):
        #Recupero le variabili, faccio i controlli
        ...
        try:
            ...
            self._view._txtResults.controls.clear()
            self._view._txtResults.controls.append(
                ft.Text("Il valore di t deve essere un valore intero positivo", color="red")
            )
            self._view.update_page()
            return
